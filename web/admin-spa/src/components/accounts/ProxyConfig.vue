  ...
  <div class="space-y-4">
    ...
    >
      <div class="mb-3 flex items-start gap-3">
        ...
        <div class="flex-1">
          <p class="text-sm text-gray-700 dark:text-gray-300">
            {{ t('accounts.form.proxyDescription') }}
          </p>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ t('accounts.form.proxyWarning') }}
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="relative">
          <input
            ...
            :placeholder="t('accounts.form.proxyUrlExample')"
            type="text"
            @input="handleInput"
        ...
        <p v-else-if="parseSuccess" class="mt-1 text-xs text-green-500">
          ...
          {{ t('accounts.form.proxyConfigFilled') }}
        </p>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{
            t('accounts.form.hostAddress')
          }}</label>
          <input
            v-model="proxy.host"
            class="form-input w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-400"
            :placeholder="t('accounts.form.hostExample')"
            type="text"
          />
        </div>
        <div>
          <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{
            t('accounts.form.port')
          }}</label>
          <input
            v-model="proxy.port"
            class="form-input w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-400"
            :placeholder="t('accounts.form.portExample')"
            type="number"
          />
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="showAuth" class="grid grid-cols-2 gap-4">
          <div>
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{
              t('accounts.form.username')
            }}</label>
            <input
              v-model="proxy.username"
              class="form-input w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-400"
              :placeholder="t('accounts.form.proxyUsername')"
              type="text"
            />
          </div>
          <div>
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">{{
              t('accounts.form.password')
            }}</label>
            <div class="relative">
              <input
                v-model="proxy.password"
                class="form-input w-full border-gray-300 pr-10 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 dark:placeholder-gray-400"
                :placeholder="t('accounts.form.proxyPassword')"
...
function parseProxyUrl() {
  ...
  try {
    ...
    if (!match) {
      ...
      parseError.value = t('accounts.form.invalidProxyUrlFormat')
      return
    }
    ...
    // 解析代理URL失败
    parseError.value = t('accounts.form.parseFailed')
  }
}
