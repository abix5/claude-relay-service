  ...
  <div class="space-y-6">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-2xl font-semibold text-gray-900">{{ t('user.apiKeysManager.title') }}</h1>
        <p class="mt-2 text-sm text-gray-700">
          {{ t('user.apiKeysManager.description') }}
        </p>
      </div>
      <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none">
        ...
        >
          ...
          {{ t('user.apiKeysManager.createButton') }}
        </button>
      </div>
    </div>
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          <p class="text-sm text-yellow-700">
            {{ t('user.apiKeysManager.maxKeysReached', { max: maxApiKeys }) }}
          </p>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="py-12 text-center">
      ...
      <p class="mt-2 text-sm text-gray-500">{{ t('user.apiKeysManager.loadingKeys') }}</p>
    </div>
            ...
            <div class="flex items-center space-x-2">
              ...
              <div class="text-right text-xs text-gray-500">
                <div>
                  ...
                  {{ t('user.apiKeysManager.requests') }}
                </div>
              ...
              <div class="flex items-center space-x-1">
                <button
                  ...
                  :title="t('user.apiKeysManager.viewApiKey')"
                ...
                <button
                  ...
                  :title="t('user.apiKeysManager.deleteApiKey')"
    ...
    <div v-else class="py-12 text-center">
      ...
      <h3 class="mt-2 text-sm font-medium text-gray-900">
        {{ t('user.apiKeysManager.noApiKeys') }}
      </h3>
      <p class="mt-1 text-sm text-gray-500">{{ t('user.apiKeysManager.getStarted') }}</p>
      <div class="mt-6">
        <button
    ...
    <ConfirmModal
      ...
      :confirm-text="t('user.apiKeysManager.deleteButton')"
      :message="t('user.apiKeysManager.deleteConfirmMessage', { name: selectedApiKey?.name })"
      :show="showDeleteModal"
      :title="t('user.apiKeysManager.deleteConfirmTitle')"
...
const loadApiKeys = async () => {
  ...
  try {
    ...
    showToast(t('user.apiKeysManager.loadFailed'), 'error')
  } finally {
    loading.value = false
  }
}
...
const handleDeleteConfirm = async () => {
  try {
    ...
    if (result.success) {
      showToast(t('user.apiKeysManager.deleteSuccess'), 'success')
      await loadApiKeys()
    }
  } catch (error) {
    showToast(t('user.apiKeysManager.deleteFailed'), 'error')
  } finally {
